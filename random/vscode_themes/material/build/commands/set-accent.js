...
const command = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // Const autoDetectColorScheme = workspace.getConfiguration().get<boolean>('window.autoDetectColorScheme');
    // const currentColorScheme = workspace.getConfiguration().get<string>('workbench.colorTheme');
    // Const preferredDarkTheme = workspace.getConfiguration().get<string>('workbench.preferredDarkColorTheme');
    // const preferredLightTheme = workspace.getConfiguration().get<string>('workbench.preferredLightColorTheme');
    const currentThemeID = vscode_1.workspace.getConfiguration().get('workbench.colorTheme');
    const isMaterialTheme = pkg.contributes.themes.some(theme => [currentThemeID].includes(theme.label));
    if (!isMaterialTheme) {
        return;
    }
    // If (autoDetectColorScheme) {
    //   currentThemeID = `[${preferredDarkTheme}][${preferredLightTheme}][${currentColorScheme}]`;
    // } else {
    //   currentThemeID = `[${currentColorScheme}]`;
    // }
    const themeConfig = extension_manager_1.extensionManager.getConfig();
    ...
    if (accent === PURGE_KEY) {
        ...
        const { '[Material Theme*]': _ } = currentColorCustomizationsConfig, rest = __rest(currentColorCustomizationsConfig, ['[Material Theme*]']);
        config = rest;
    }
    else {
        config = Object.assign(Object.assign({}, currentColorCustomizationsConfig), { 
            // [`${currentThemeID}`]: {
            '[Material Theme*]': Object.assign({}, getThemeColorCustomizationsConfig(themeConfig.accents[accent])) });
    }
